...
void notes_window_init_preview(NotesWindow *win) {
    ...
    if (elen > 0) {
        ...
        g_string_append(js,
            ...
            /* Clean up mermaid temp elements from previous renders */
            "document.querySelectorAll('[id^=\"svg-mm-\"],[id^=\"dsvg-\"],[id^=\"dmm-\"]').forEach(function(e){"
            "if(!e.closest('#content'))e.remove();});\n"
            "_mmPending=false;\n"
            "var mmEls=document.querySelectorAll('.mermaid');\n"
            "if(mmEls.length===0)return;\n"
            "if(mmEls.length>20)mmEls=Array.prototype.slice.call(mmEls,0,20);\n"
            "_mmPending=true;\n"
            "var done=0,total=mmEls.length;\n"
            "mmEls.forEach(function(el){\n"
            "var c=el.textContent,id='svg-'+el.id;\n"
            "try{mermaid.render(id,c).then(function(res){"
            "el.innerHTML=res.svg;done++;if(done>=total)_mmPending=false;"
